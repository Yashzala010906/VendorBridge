...
- **Email:** pluggable — SMTP (nodemailer), Resend API, or mock fallback
...
| `RESEND_API_KEY` | optional | Enables real invoice emails via Resend |
| `SMTP_HOST` | optional | SMTP server hostname (enables nodemailer) |
| `SMTP_PORT` | optional | SMTP port (default `587`) |
| `SMTP_USER` | optional | SMTP username |
| `SMTP_PASS` | optional | SMTP password |
| `SMTP_SECURE` | optional | Set `true` for TLS on port 465 |
...
- **Email sending** supports SMTP (nodemailer), Resend API, or a mock fallback. Configure
  `SMTP_HOST` or `RESEND_API_KEY` in `.env.local` to enable real delivery.
